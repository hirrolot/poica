...
A: `#define POICA_USE_PREFIX` before `#include <poica.h>`. All the public `camelCase`ed and `PascalCase`ed macros will be prefixed with `poica` (`match` -> `poicaMatch`, `DefRes` -> `PoicaDefRes`, ...) in the current translation unit.
